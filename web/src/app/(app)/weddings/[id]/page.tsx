...
export default function WeddingDetailPage({
  ...
  async function refreshGuests() {
    try {
      const data = await listWeddingGuests(id);
      setGuests(data);
      setGuestError(null);
    } catch (err) {
  ...
  async function refreshGuestSummary() {
    try {
      const data = await getGuestSummary(id);
        ...
        <SectionHeading
          action={
            <div className="flex flex-wrap items-center gap-2">
              <Link href={`/import?type=guests&weddingId=${id}`} className={btnSecondarySm}>
                Import from CSV
              </Link>
              <button
                type="button"
              ...
              <button
                type="button"
                onClick={() => setGuestsExpanded((e) => !e)}
                className={btnSecondarySm}
              >
                {guestsExpanded ? "Collapse" : "Expand"}
              </button>
            </div>
          }
        >
          ...
          <div className="mt-4">
            {showGuestForm && (
              ...
              >
                ...
                {guestError && (
                  <p className="text-sm text-rose-700 sm:col-span-4">{guestError}</p>
                )}
                <div className="flex items-end sm:col-span-4">
            ...
            {!showGuestForm && guestError && (
              <p className="mb-2 text-sm text-rose-700">{guestError}</p>
            )}
